...
const ChatBotComponent = () => {
    ...
    return (
        <>
            {isOpen && (
                <div className='fixed bottom-10 right-5 w-[clamp(220px,50vw,400px)] h-[76vh] z-40 bg-white rounded-[12px] backdrop-blur-[4px] shadow overflow-hidden flex flex-col'>
                    ...
                    <div ref={messageContainerRef} className='flex-1 px-4 py-2 overflow-y-auto flex flex-col'>
                        {
                            chatStore.chatList.map(({ role, text, hideInChat }, index) => (
                                !hideInChat ? ( 
                                  role === "user" ? (
                                    <div key={index} className='bg-zinc-700 text-white p-3 ms-auto w-[60%] whitespace-pre-line text-[14px] rounded-[12px] rounded-tr-none my-1'>
                                      <p>{text}</p>
                                    </div>
                                  ) : (
                                    <div key={index} className='bg-zinc-200 p-3 mr-auto w-[60%] whitespace-pre-line text-[14px] rounded-[12px] rounded-tl-none my-1'>
                                      <p>{text}</p>
                                    </div>
                                  )
                                ) : null
                    ...
                    <input ref={queryRef} type="text" className='w-full bg-zinc-700 h-[60px] rounded-[12px] outline-none border-none text-[14px] pl-8 pr-[70px] text-white placeholder:text-zinc-400' placeholder='Ask anything' />
                </div>
            )}
            {
                !isOpen 
                ?
                <button onClick={openChat} type='button' title='Chat' className='w-[60px] shadow-sm bg-zinc-700 aspect-square rounded-full flex cursor-pointer items-center justify-center fixed bottom-10 right-5 z-50 text-xl text-white'><IoChatboxOutline /></button>
                :
                <button onClick={handleSendMessage} type='button' title='Send' className='w-[60px] aspect-square cursor-pointer rounded-full flex items-center justify-center fixed bottom-10 right-5 z-50 text-xl text-white'><IoMdSend /></button>
            }
        </>
    )
}
